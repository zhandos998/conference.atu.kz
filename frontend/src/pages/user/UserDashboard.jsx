...
export default function UserDashboard({ onLogout }) {
  ...
  const openNotice = (title, msg) => {
    setNoticeModal({ open: true, title, message: msg });
  };

  const closeNotice = () => {
    setNoticeModal({ open: false, title: "", message: "" });
  };
  ...
  const goToEdit = () => {
    ...
    if (selectedApplication.status !== "revision") {
      openNotice(
        ...
        'Изменить заявку можно только при статусе "На доработку".'
      );
      return;
    }
  ...
  const buildPayload = () => {
    ...
    Object.entries(form).forEach(([key, value]) => {
      ...
      payload.append(
        key,
        key === "hotel_booking_needed" ? Number(value) : value
      );
    });
  ...
  const submitPaymentReceipt = async (e) => {
    ...
    try {
      await api.post(
        `/applications/${selectedApplication.id}/payment-receipt`,
        payload
      );
      const { data } = await api.get(`/applications/${selectedApplication.id}`);
      setSelectedApplication(data);
  ...
  const renderApplicationForm = (onSubmit, submitLabel) => (
    <form onSubmit={onSubmit}>
      <div className="grid">
        ...
        <div className="field">
          ...
          <input
            ...
            onChange={(e) =>
              setForm({ ...form, supervisor_academic_degree: e.target.value })
            }
          />
        </div>
        <div className="field">
      ...
      <div className="inline-actions">
        ...
        <button className="btn-secondary" type="button" onClick={goToList}>
          Назад к списку
        </button>
      </div>
    </form>
  ...
  const renderList = () => (
    <>
      <div className="inline-actions">
        <button className="btn-primary" type="button" onClick={goToCreate}>
          Добавить заявку
        </button>
      </div>
          ...
          {applications.map((app) => (
            <div key={app.id} className="app-item">
              ...
              <p>
                ...
                <span
                  className={statusClass[app.status] || statusClass.pending}>
  ...
  const renderDetail = () => {
    ...
    const reportFileUrl = selectedApplication.file_path
      ? toReportFileUrl(selectedApplication.file_path)
      : "";
    ...
    return (
      <>
        <div className="inline-actions">
          ...
          <button className="btn-primary" type="button" onClick={goToEdit}>
            Изменить заявку
          </button>
        </div>
        ...
        <div className="app-item" style={{ marginTop: 12 }}>
          <p>
            <strong>Ф.И.О.:</strong> {selectedApplication.full_name}
          </p>
          <p>
            <strong>Место учебы/работы и должность:</strong>{" "}
            {selectedApplication.organization_position}
          </p>
          <p>
            <strong>Ученая степень:</strong>{" "}
            {selectedApplication.academic_degree}
          </p>
          <p>
          ...
          <p>
            <strong>Ф.И.О. научного руководителя:</strong>{" "}
            {selectedApplication.supervisor_full_name}
          </p>
          <p>
            <strong>Должность научного руководителя:</strong>{" "}
            {selectedApplication.supervisor_organization_position}
          </p>
          <p>
            <strong>Степень научного руководителя:</strong>{" "}
            {selectedApplication.supervisor_academic_degree}
          </p>
          <p>
            <strong>Название доклада:</strong>{" "}
            {selectedApplication.report_title}
          </p>
          <p>
            <strong>Направление:</strong> {selectedApplication.direction}
          </p>
          <p>
            <strong>Форма участия:</strong>{" "}
            {selectedApplication.participation_form}
          </p>
          <p>
            <strong>Бронирование гостиницы:</strong>{" "}
            {selectedApplication.hotel_booking_needed ? "Да" : "Нет"}
          </p>
          <p>
          ...
          <p>
            ...
            <span
              className={
                statusClass[selectedApplication.status] || statusClass.pending
              }>
          ...
          <p>
            ...
            {selectedApplication.moderator_comment || "-"}
          </p>
        </div>
  ...
  return (
    <>
      <AppLayout
        ...
        {view === "create" &&
          renderApplicationForm(submitCreate, "Отправить заявку")}
        {view === "detail" && renderDetail()}
        {view === "edit" &&
          renderApplicationForm(submitEdit, "Сохранить изменения")}
      </AppLayout>
      ...
      <Modal
        ...
        onClose={closeNotice}
        actions={
          <button className="btn-primary" type="button" onClick={closeNotice}>
            Понятно
          </button>
        }>
